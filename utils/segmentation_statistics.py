...
class SegmentationStatistics:
    ...
    def truth_volume(self):
        return np.sum(self.truth) * np.prod(self.zoom)

    def prediction_volume(self):
        return np.sum(self.prediction) * np.prod(self.zoom)

    def total_volume(self):
        return np.prod(self.prediction.size)

    def to_dict(self):
        return {'Dice': self.dice.round(4),
                'Jaccard': self.jaccard.round(4),
                'Sensitivity': self.sensitivity.round(4),
                'Specificity': self.specificity.round(4),
                'Precision': self.precision.round(4),
                'Accuracy': self.accuracy.round(4),
                'Mean Surface Distance': self.mean_surface_distance.round(4),
                'Hausdorff Distance': self.hausdorff_distance.round(4),
                'Volume Difference': self.volume_difference.round(4)}
    ...
    def _volume_difference(self):
        return (self.prediction_volume() - self.truth_volume())/self.truth_volume()
